...
if(wpsc_cart_item_count() < 1) :
   _e('Oops, there is nothing in your cart.', 'colabsthemes') . "<a href=".get_option("product_list_url").">" . __('Please visit our shop', 'colabsthemes') . "</a>";
   return;
endif;
?>
<div id="checkout_page_container">
<h3><?php _e('Please review your order', 'colabsthemes'); ?></h3>
<table class="checkout_cart">
   <tr class="header">
      <th colspan="2" ><?php _e('Product', 'colabsthemes'); ?></th>
      <th><?php _e('Quantity', 'colabsthemes'); ?></th>
      <th><?php _e('Price', 'colabsthemes'); ?></th>
      ...
      <tr class="product_row product_row_<?php echo wpsc_the_cart_item_key(); ?> <?php echo $alt_class;?>">
         ...
         ?>
            <div class="item_no_image">
               ...
               <span><?php _e('No Image','colabsthemes'); ?></span>
         ...
         <td class="wpsc_product_quantity wpsc_product_quantity_<?php echo wpsc_the_cart_item_key(); ?>">
            <form action="<?php echo get_option('shopping_cart_url'); ?>" method="post" class="adjustform qty">
               ...
               <input type="submit" value="<?php _e('Update', 'colabsthemes'); ?>" name="submit" />
            </form>
         </td>
         ...
         <td class="wpsc_product_remove wpsc_product_remove_<?php echo wpsc_the_cart_item_key(); ?>">
            <form action="<?php echo get_option('shopping_cart_url'); ?>" method="post" class="adjustform remove">
               ...
               <input type="hidden" name="key" value="<?php echo wpsc_the_cart_item_key(); ?>" />
               <input type="hidden" name="wpsc_update_quantity" value="true" />
               <input type="submit" value="<?php _e('Remove', 'colabsthemes'); ?>" name="submit" />
            </form>
         </td>
      </tr>
   <?php endwhile; ?>
   ...
   if(wpsc_uses_coupons()): ?>
      ...
      <?php if(wpsc_coupons_error()): ?>
         <tr class="wpsc_coupon_row wpsc_coupon_error_row"><td><?php _e('Coupon is not valid.', 'colabsthemes'); ?></td></tr>
      <?php endif; ?>
      <tr class="wpsc_coupon_row">
         <td colspan="2"><?php _e('Enter coupon code', 'colabsthemes'); ?> :</td>
         <td  colspan="4" class="coupon_code">
            <form  method="post" action="<?php echo get_option('shopping_cart_url'); ?>">
               ...
               <input type="submit" value="<?php _e('Update', 'colabsthemes') ?>" />
            </form>
         </td>
      </tr>
      <tr class="wpsc_total_before_shipping">
	      <td colspan="3"><?php _e('Cost before shipping:','colabsthemes'); ?></td>
  ...
  <?php if(wpsc_uses_shipping()): ?>
   ...
   <?php if(wpsc_uses_shipping()) : ?>
      <h2><?php _e('Calculate Shipping Price', 'colabsthemes'); ?></h2>
      <table class="productcart">
         <tr class="wpsc_shipping_info">
            <td colspan="5">
               <?php _e('Please choose a country below to calculate your shipping costs', 'colabsthemes'); ?>
            </td>
         </tr>
            ...
            <?php if (wpsc_have_valid_shipping_zipcode()) : ?>
                  <tr class='wpsc_update_location'>
                     <td colspan='5' class='shipping_error' >
                        <?php _e('Please provide a Zipcode and click Calculate in order to continue.', 'colabsthemes'); ?>
                     </td>
                  </tr>
            <?php else: ?>
               <tr class='wpsc_update_location_error'>
                  <td colspan='5' class='shipping_error' >
                     <?php _e('Sorry, online ordering is unavailable to this destination and/or weight. Please double check your destination details.', 'colabsthemes'); ?>
                  </td>
               </tr>
            <?php endif; ?>
         <?php endif; ?>
         ...
         <?php if (wpsc_have_morethanone_shipping_quote()) :?>
            <?php while (wpsc_have_shipping_methods()) : wpsc_the_shipping_method(); ?>
                  ...
                  <tr class='wpsc_shipping_header'><td class='shipping_header' colspan='5'><?php echo wpsc_shipping_method_name().__(' - Choose a Shipping Rate', 'colabsthemes'); ?> </td></tr>
   ...
   <?php do_action('wpsc_before_form_of_shopping_cart'); ?>
	...
	<?php if ( wpsc_show_user_login_form() && !is_user_logged_in() ): ?>
			<p><?php _e('You must sign in or register with us to continue with your purchase', 'colabsthemes');?></p>
			<div class="wpsc_registration_form">
				
				<fieldset class='wpsc_registration_form'>
					<h2><?php _e( 'Sign in', 'colabsthemes' ); ?></h2>
					<?php
					$args = array(
						'remember' => false,
					...
					wp_login_form( $args );
					?>
					<div class="wpsc_signup_text"><?php _e('If you have bought from us before please sign in here to purchase', 'colabsthemes');?></div>
				</fieldset>
			</div>
	<?php endif; ?>	
    ...
    <?php if(wpsc_show_user_login_form()):
		...
		<div class="wpsc_registration_form">
	        ...
	        <fieldset class='wpsc_registration_form wpsc_right_registration'>
	        	<h2><?php _e('Join up now', 'colabsthemes');?></h2>
				...
				<label><?php _e('E-mail', 'colabsthemes'); ?>:</label>
	            <input type="text" name="user_email" id="user_email" value="<?php echo attribute_escape(stripslashes($user_email)); ?>" size="20" /><br />
	            
	            <div class="wpsc_signup_text"><?php _e('Signing up is free and easy! please fill out your details your registration will happen automatically as you checkout. Don\'t forget to use your details to login with next time!', 'colabsthemes');?></div>
	        </fieldset>
        ...
        <?php if(wpsc_checkout_form_is_header() == true){
               ...
               <tr class='same_as_shipping_row'>
                  ...
                  elseif(isset($_POST['submit']) && !isset($_POST['shippingSameBilling']))
                   ...
                   ?>
					<label for='shippingSameBilling'><?php _e('Same as billing address:','colabsthemes'); ?></label>
					<input type='checkbox' value='true' name='shippingSameBilling' id='shippingSameBilling' <?php echo $checked; ?> />
					<br/><span id="shippingsameasbillingmessage"><?php _e('Your order will be shipped to the billing address', 'colabsthemes'); ?></span>
                  </td>
               </tr>
               <?php endif;
            ...
            <?php
               ...
               "<div class='wpsc_email_address'>
                  <p class='" . wpsc_checkout_form_element_id() . "'>
                     ...
                     " . __('Enter your email address', 'colabsthemes') . "
                     </label>
...
?>
      ...
      <tr>
         <td><label for='how_find_us'><?php _e('How did you find us' , 'colabsthemes'); ?></label></td>
         <td>
            <select name='how_find_us'>
               <option value='Word of Mouth'><?php _e('Word of mouth' , 'colabsthemes'); ?></option>
               <option value='Advertisement'><?php _e('Advertising' , 'colabsthemes'); ?></option>
               <option value='Internet'><?php _e('Internet' , 'colabsthemes'); ?></option>
               <option value='Customer'><?php _e('Existing Customer' , 'colabsthemes'); ?></option>
            </select>
         </td>
      </tr>
      <?php endif; ?>
      ...
      <?php if(wpsc_gateway_count() > 1): // if we have more than one gateway enabled, offer the user a choice ?>
         ...
         <td colspan='2' class='wpsc_gateway_container'>
            <h3><?php _e('Payment Type', 'colabsthemes');?></h3>
      ...
      <?php if(wpsc_uses_shipping()) : ?>
	      <tr>
	         <td class='wpsc_total_price_and_shipping'colspan='2'>
	            <h4><?php _e('Review and purchase','colabsthemes'); ?></h4>
	         </td>
	      </tr>
	
	      <tr class="total_price total_shipping">
	         <td class='wpsc_totals'>
	            <?php _e('Total Shipping', 'colabsthemes'); ?>:
	         </td>
	         <td class='wpsc_totals'>
   ...
   <tr class='total_price'>
      <td class='wpsc_totals'>
      <?php _e('Total Price', 'colabsthemes'); ?>:
      </td>
      <td class='wpsc_totals'>
      ...
      <div class='wpsc_make_purchase'>
         <span>
            ...
            <?php endif; ?>
               ...
               <input type='submit' value='<?php _e('Purchase', 'colabsthemes');?>' name='submit' class='make_purchase wpsc_buy_button' />
         </span>
      </div>
